...
<template>
	<v-card>
		<v-card-title>
			<v-icon small class="mr-1">mdi-polymer</v-icon><span v-if="macroMode"> {{ $t('list.macro.caption') }} </span><span v-if="!macroMode"> Sys </span>
			<v-spacer></v-spacer>
			<span v-show="isConnected" class="subtitle-2">{{ currentDirectory }}</span>
		</v-card-title>

		<v-card-text class="pa-0 cardRLM" v-show="loading || filelist.length || !isRootDirectory">
			<v-progress-linear v-show="loading" :indeterminate="true" class="my-0"></v-progress-linear>

			<v-list dense style="max-height: 300px" class="pt-0 overflow-auto" :key="reloadKey">
				...
				<v-list-item v-for="item in filelist" :disabled="!item.isDirectory && !isMatchedFile(item.displayName)" :key="item.name" @click="itemClick(item)">
					<v-list-item-avatar :size="32">
						<v-icon v-if="isMatchedFile(item.displayName)" small class="mr-1" :class="item.isDirectory ? 'grey lighten-1 white--text' : 'red white--text'">
							{{ item.isDirectory ? 'mdi-folder' : 'mdi-file' }}
						</v-icon>
						<v-icon v-if="!isMatchedFile(item.displayName)" small class="mr-1" :class="item.isDirectory ? 'grey lighten-1 white--text' : 'blue white--text'">
							{{ item.isDirectory ? 'mdi-folder' : 'mdi-file' }}
						</v-icon>
					</v-list-item-avatar>
...
export default {
	...
	computed: {
		...
		...mapState('machine/model', {
			mDir: state => state.directories.macros,
			sDir: state => state.directories.system,
	...
	methods: {
		...
		async itemClick(item) {
			...
			if (item.isDirectory) {
				...
				this.$emit('currDir', this.directory);
			}else{
				this.$emit('fileClcked', item.name);
			}
		},
		async goUp() {
	...
	mounted() {
		...
		this.$emit('currDir', this.macrosDirectory); 
		if (this.isConnected) {
