...
function runCheck(){
	if(tmpSession.checkOnLoad){
		...
		bfwName = store.state.machine.model.boards[0].firmwareName;
		bfwVersion = store.state.machine.model.boards[0].firmwareVersion;
		console.log("FW V: ", bfwVersion)
		console.log("FW N: ", bfwName)
		//bfwVersion = '3.4.1'
...
async function checkVerUpdate(){
	if(fwSrc() == 'gloomyandy'){
		...
		if(relJSONcheck){
			...
			var relJSONcheckFiltered = relJSONcheck.sort((a: any, b: any) => (a.published_at < b.published_at) ? 1 : -1)
			relJSONcheckFiltered = relJSONcheckFiltered.filter((item: { tag_name: string; }) => (item.tag_name >= "v3.3"));
		...
		if(relJSONcheck2){
			//re-order by date release published
			var relJSONcheckFiltered2 = relJSONcheck2.sort((a: any, b: any) => (a.published_at < b.published_at) ? 1 : -1)
			relJSONcheckFiltered2 = relJSONcheckFiltered2.filter((item: { tag_name: string; }) => (item.tag_name >= "3.3" && !(item.tag_name.charAt(0)=='v')));
...
/**
 * getByGitAPI
 * @param apiURLSuffix string - Git URL SUffix
 * @param gitUName string - Git User Name
 * @param gitRepoName string - Git Repo Name
 * @returns JSON Object
*/
async function getByGitAPI(apiURLSuffix: string, gitUName: string, gitRepoName: string){
